...
public class OkexMarketDataService extends OkexMarketDataServiceRaw implements MarketDataService {
  ...
  public Ticker getTicker(Instrument instrument, Object... args) throws IOException {
    List<OkexTicker> tickers = getOkexTicker(OkexAdapters.adaptInstrumentToOkexInstrumentId(instrument)).getData();
    return tickers.isEmpty() ? null : OkexAdapters.adaptTicker(tickers.get(0));
  }

  @Override
  public List<Ticker> getTickers(Params params) throws IOException {
    return getOkexTickers(SPOT, null).getData().stream().map(OkexAdapters::adaptTicker).collect(Collectors.toList());
  }
}
